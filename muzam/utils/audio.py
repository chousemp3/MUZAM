...
class AudioProcessor:
    ...
    def load_audio(self, file_path: str, offset: float = 0.0, duration: Optional[float] = None) -> Tuple[np.ndarray, int]:
        """
        ...
        try:
            ...
            self.logger.debug(f"Loaded audio: {file_path.name}, duration: {len(audio_data)/sr:.2f}s")
    ...
    def record_from_microphone(self, duration: int, device: Optional[int] = None) -> np.ndarray:
        """
    ...
    def save_audio(self, audio_data: np.ndarray, file_path: str, sample_rate: Optional[int] = None) -> bool:
        """
