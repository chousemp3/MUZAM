...
class FingerprintGenerator:
    ...
    def generate(self, audio_data: np.ndarray, sample_rate: int) -> Fingerprint:
        ...
        try:
            ...
            spectral_hashes = self._generate_spectral_fingerprint(
            ...
            timestamps = self._generate_timestamps(
                len(all_hashes), len(audio_data), sample_rate
            )
    ...
    def _generate_chromaprint(
        self, audio_data: np.ndarray, sample_rate: int
    ) -> List[str]:
        """Generate chromaprint-style fingerprint"""
        # Calculate chroma features
        chroma = librosa.feature.chroma_stft(
            y=audio_data, sr=sample_rate, hop_length=self.hop_length, n_fft=self.n_fft
        )
    ...
    def _generate_spectral_fingerprint(
        self, audio_data: np.ndarray, sample_rate: int
    ) -> List[str]:
        ...
        hashes = []
        min_len = min(
            len(spectral_centroids), len(spectral_rolloff), len(spectral_bandwidth)
        )
        ...
        for i in range(min_len - 1):
            features = np.array(
                [
                    ...
                    spectral_centroids[i + 1],
                    spectral_rolloff[i],
                    spectral_rolloff[i + 1],
                    spectral_bandwidth[i],
                    spectral_bandwidth[i + 1],
                ]
            )
    ...
    def _generate_mfcc_fingerprint(
        self, audio_data: np.ndarray, sample_rate: int
    ) -> List[str]:
        ...
        for i in range(mfccs.shape[1] - 1):
            features = np.concatenate(
                [mfccs[:, i], mfccs[:, i + 1], delta_mfccs[:, i], delta_mfccs[:, i + 1]]
            )
    ...
    def _generate_timestamps(
        self, num_hashes: int, audio_length: int, sample_rate: int
    ) -> List[float]:
