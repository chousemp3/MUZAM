...
class DatabaseManager:
    ...
    def add_song(self, song: Song, fingerprint: Fingerprint) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for i, (hash_val, timestamp) in enumerate(
                    zip(fingerprint.hash_values, fingerprint.time_stamps)
                ):
                    cursor.execute(
                        """
    ...
    def search_fingerprint(
        self, fingerprint: Fingerprint, max_results: int = 10
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                min_matches = max(
                    1, len(fingerprint.hash_values) // 10
                )  # At least 10% match

                cursor.execute(
                    query, fingerprint.hash_values + [min_matches, max_results]
                )
                ...
                for row in results:
                    song_id, title, artist, album, year, match_count, unique_matches = (
                        row
                    )
                    ...
                    self._record_recognition_stats(
    ...
    def _calculate_confidence(
        self, match_count: int, unique_matches: int, total_query_hashes: int
    ) -> float:
    ...
    def _record_recognition_stats(
        self, song_id: int, recognition_time: float, confidence: float
    ):
    ...
    def batch_add_songs(
        self, songs_and_fingerprints: List[Tuple[Song, Fingerprint]]
    ) -> List[int]:
    ...
    def delete_song(self, song_id: int) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    "DELETE FROM fingerprint_hashes WHERE song_id = ?", (song_id,)
                )

                # Delete recognition stats
                cursor.execute(
                    "DELETE FROM recognition_stats WHERE song_id = ?", (song_id,)
                )
